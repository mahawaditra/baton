...
export default async function Home() {
  const settings = await prisma.loanSetting.findFirst({
    select: {
      signatoryPhone: true,
      signatoryPhonePublic: true,
      signatoryLineAddFriendUrl: true,
      signatoryLineAddFriendPublic: true,
    },
  });
  ...
  const lineAddFriendUrl =
    settings?.signatoryLineAddFriendPublic && settings.signatoryLineAddFriendUrl
      ? settings.signatoryLineAddFriendUrl
      : undefined;

  return (
    <div className="min-h-screen bg-background">
      <section className="relative isolate -mt-[env(safe-area-inset-top,0px)] flex min-h-[100vh] items-end justify-center overflow-hidden bg-hero-bg pt-[env(safe-area-inset-top,0px)] select-none">
        <HeroMarquee />
        <div
          aria-hidden
      ...
      <div className="relative isolate">
        <section className="border-t border-border bg-surface px-6 py-16">
          <div className="mx-auto max-w-7xl">
            ...
            {(whatsappNumber || lineAddFriendUrl) && (
              <div className="mt-3 flex flex-wrap justify-center gap-3">
                ...
                {lineAddFriendUrl && (
                  <ContactLink href={lineAddFriendUrl} icon={LineIcon}>
                    Chat via LINE
                  </ContactLink>
                )}
              </div>
